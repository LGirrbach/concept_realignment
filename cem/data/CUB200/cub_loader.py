...
class CUBDatasetImageLevel(Dataset):
    def __init__(self, pkl_file_paths: list, root: str, split: str, preprocess, uncertain_label=False):
        ...
        img_paths = [
            data['img_path'].replace(
                '/juice/scr/scr102/scr/thaonguyen/CUB_supervision/datasets/CUB_200_2011',
                '/dss/dsshome1/04/go25kod3/projects/concept_realignment/cem/data/CUB200'
        ...
        print(pkl_file_paths, len(self.image_ids), img_paths[0], len(img_paths))
        print(next(iter(self.images.values())))
    ...
    def __getitem__(self, idx):
        ...
        if self.path_transform == None:
            ...
            try:
                ...
                # print(self.root_dir)
                # print(img_path.split('/')[idx+1:])
                # print(idx)
                # print(img_path.split('/'))
                img_path = self.root_dir + '/' + '/'.join(img_path.split('/')[idx+1:])
