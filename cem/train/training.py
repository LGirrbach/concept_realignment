...
def train_model(
    ...
    if (project_name) and result_dir and (
        ...
        with wandb.init(
            ...
            checkpoint_savedir = os.path.join(result_dir, 'checkpoint_savedir')
            Path(checkpoint_savedir).mkdir(parents=True, exist_ok=True)
            checkpoint_callback = ModelCheckpoint(dirpath=checkpoint_savedir, save_top_k=1, monitor=config["early_stopping_monitor"], mode=config["early_stopping_mode"])
...
#                 track_grad_norm=2,
            ...
            if (not rerun) and os.path.exists(model_saved_path):
                # Then we simply load the model and proceed
                print("\tFound cached model... loading it. Path = ", model_saved_path)
                ...
                print("LOADING MODEL FROM THE BEST PATH")
...
def train_independent_and_sequential_model(
    ...
    with enter_obj as run:
        ...
        if (not rerun) and chpt_exists:
            ...
            print("Ind Model checkpoint path = ", ind_model_saved_path)
            print("Seq Model checkpoint path = ", seq_model_saved_path)
            ...
            print("LOADING MODEL FROM THE BEST PATH")
            ind_c2y_model.load_state_dict(torch.load(checkpoint_callback.best_model_path)['state_dict'])
            ...
            print("LOADING MODEL FROM THE BEST PATH")
            seq_c2y_model.load_state_dict(torch.load(checkpoint_callback.best_model_path)['state_dict'])
