...
class PatchedBCELoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        ...
        pointwise_loss = F.binary_cross_entropy(input, target.to(input.device), weight=self.weight.to(input.device), reduction='none').flatten()
...
class ConceptBottleneckModel(pl.LightningModule):
    ...
    def _forward(
        ...
        if self.sigmoidal_prob or self.bool:
            print('self.sigmoidal_prob or self.bool = TRUE')
            if self.extra_dims:
                print('self.extra_dims = TRUE')
                ...
                print('self.extra_dims = FALSE')
                c_pred = self.sig(latent)
                c_sem = c_pred
        else:
            print('self.sigmoidal_prob or self.bool = FALSE')
        ...
        if output_embeddings or (
            ...
            if pos_embeddings.isnan().any():
                print('POS EMBEDDINGS HAS NAN')
            if neg_embeddings.isnan().any():
                print('NEG EMBEDDINGS HAS NAN')
        ...
        if (intervention_idxs is None) and (c is not None) and (
            ...
            if not self.include_certainty:
                ...
                if torch.any(indices):
                    ...
                    if intervention_idxs.isnan().any():
                        print('intervention_idxs HAS NAN')
                    if c_int.isnan().any():
                        print('c_int HAS NAN')
                    if indices.isnan().any():
                        print('indices HAS NAN')
            else:
                print('INSIDE ELSE')
                prior_distribution = self._prior_int_distribution(
                    c=c,
            ...
            if np.isnan(intervention_idxs).any():
                    print('intervention_idxs HAS NAN')
            if c_int.isnan().any():
                    print('c_int HAS NAN')
    ...
    def _run_step(
        ...
        pred_labels = torch.argmax(y_logits, dim=-1)
        # print(pred_labels)
